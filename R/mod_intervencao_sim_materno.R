#' intervencao_sim_materno UI Function
#'
...
mod_intervencao_sim_materno_ui <- function(id){
  ns <- NS(id)
  tagList(
    fluidPage(
...
#' intervencao_sim_materno Server Functions
#'
#' @noRd
mod_intervencao_sim_materno_server <- function(id, opcoes_usuario){
  moduleServer( id, function(input, output, session){
    ...
    titulo <- eventReactive(input$gerar_graficos, {
      ifelse(opcoes_usuario$nivel_geografico=="PR","(SIM MATERNO) Análise de impacto na tendência no Paraná",
             ifelse(opcoes_usuario$nivel_geografico=="macro",paste("(SIM MATERNO) Análise de impacto na tendência na Macrorregião",opcoes_usuario$escolha_usuario),
                    ifelse(opcoes_usuario$nivel_geografico=="micro",paste("(SIM MATERNO) Análise de impacto na tendência na Regional de Saúde",opcoes_usuario$escolha_usuario),
      ...
      ifelse(opcoes_usuario$nivel_geografico=="PR","(SIM MATERNO) Análise de impacto na tendência por raça do mortalidade materna no Paraná",
             ifelse(opcoes_usuario$nivel_geografico=="macro",paste("(SIM MATERNO) Análise de impacto na tendência por raça do mortalidade materna na Macrorregião",opcoes_usuario$escolha_usuario),
                    ifelse(opcoes_usuario$nivel_geografico=="micro",paste("(SIM MATERNO) Análise de impacto na tendência por raça do mortalidade materna  na Regional de Saúde",opcoes_usuario$escolha_usuario),
                           paste("(SIM MATERNO) Análise de impacto na tendência por raça do mortalidade materna no Município de", opcoes_usuario$escolha_usuario))))
    })
    ...
    output$relatorio <- downloadHandler(
      ...
      filename <-  "Análise de resíduos (SIM-Materno).html",
      ...
      content = function(file) {
        ...
        titulo_relatorio <- eventReactive(input$gerar_graficos, {
          ifelse(is.na(opcoes_usuario$date_intervention[2]),
                 paste("Análise de resíduos, dados do SIM-Materno e data de intervenção:",
                       opcoes_usuario$date_intervention[1]),
                 paste("Análise de resíduos, dados do SIM-Materno e datas de intervenção:",
...
# mod_intervencao_sim_materno_server("intervencao_sim_materno_1")
