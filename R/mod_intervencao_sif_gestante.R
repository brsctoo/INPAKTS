#' intervencao_sif_gestante UI Function
#'
...
mod_intervencao_sif_gestante_ui <- function(id){
  ns <- NS(id)
  tagList(
    fluidPage(
...
#' intervencao_sif_gestante Server Functions
#'
#' @noRd
mod_intervencao_sif_gestante_server <- function(id, opcoes_usuario){
  moduleServer( id, function(input, output, session){
    ...
    titulo <- eventReactive(input$gerar_graficos, {
      ifelse(opcoes_usuario$nivel_geografico=="PR","(SÍFILIS GESTACIONAL) Análise de impacto na tendência no Paraná",
             ifelse(opcoes_usuario$nivel_geografico=="macro",paste("(SÍFILIS GESTACIONAL) Análise de impacto na tendência na Macrorregião",opcoes_usuario$escolha_usuario),
                    ifelse(opcoes_usuario$nivel_geografico=="micro",paste("(SÍFILIS GESTACIONAL) Análise de impacto na tendência na Regional de Saúde",opcoes_usuario$escolha_usuario),
...
# mod_intervencao_sif_gestante_server("intervencao_sif_gestante_1")
