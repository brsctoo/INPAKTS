...
mod_intervencao_sif_congenita_server <- function(id, opcoes_usuario){
  moduleServer( id, function(input, output, session){
    ...
    dataCategorica <- eventReactive(input$gerar_graficos, {
      dados_sif_congenita_intervention %>%
    ...
    titulo <- eventReactive(input$gerar_graficos, {
      ifelse(opcoes_usuario$nivel_geografico=="PR","(SÍFILIS CONGÊNITA) Análise de impacto na tendência no Paraná",
             ifelse(opcoes_usuario$nivel_geografico=="macro",paste("(SÍFILIS CONGÊNITA) Análise de impacto na tendência na Macrorregião",opcoes_usuario$escolha_usuario),
                    ifelse(opcoes_usuario$nivel_geografico=="micro",paste("(SÍFILIS CONGÊNITA) Análise de impacto na tendência na Regional de Saúde",opcoes_usuario$escolha_usuario),
      ...
      ifelse(opcoes_usuario$nivel_geografico=="PR","(SÍFILIS CONGÊNITA) Análise de impacto na tendência por raça no Paraná",
             ifelse(opcoes_usuario$nivel_geografico=="macro",paste("(SÍFILIS CONGÊNITA) Análise de impacto na tendência por raça na Macrorregião",opcoes_usuario$escolha_usuario),
                    ifelse(opcoes_usuario$nivel_geografico=="micro",paste("(SÍFILIS CONGÊNITA) Análise de impacto na tendência por raça na Regional de Saúde",opcoes_usuario$escolha_usuario),
                           paste("(SÍFILIS CONGÊNITA) Análise de impacto na tendência por raça no Município de", opcoes_usuario$escolha_usuario))))
    })
    ...
    output$relatorio <- downloadHandler(
      ...
      filename <-  "Análise de resíduos (Sífilis-Congênita).html",
      ...
      content = function(file) {
        ...
        titulo_relatorio <- eventReactive(input$gerar_graficos, {
          ifelse(is.na(opcoes_usuario$date_intervention[2]),
                 paste("Análise de resíduos, dados do Sífilis-Congênita e data de intervenção:",
                       opcoes_usuario$date_intervention[1]),
                 paste("Análise de resíduos, dados do Sífilis-Congênita e datas de intervenção:",
